...
class Mesh : public painlessmesh::Mesh<Connection> {
 ...
 protected:
  ...
  void init(Scheduler *scheduler, uint32_t id, WiFiMode_t cMode) {
    painlessmesh::Mesh<Connection>::init(scheduler, id, cMode);
  }

  void init(uint32_t id, WiFiMode_t cMode) { painlessmesh::Mesh<Connection>::init(id, cMode); }
