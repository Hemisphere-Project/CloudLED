...
#define TIME_SYNC_INTERVAL 1000 * TASK_MINUTE  // Time resync period
#endif
...
inline bool adopt(protocol::NodeTree mesh, protocol::NodeTree connection,
                  WiFiMode_t cMode = WIFI_MODE_APSTA) {
  auto mySubCount =
  ...
  if (cMode == WIFI_MODE_AP) {
    Log(logger::S_TIME, "adopt(): Won't adopt i am AP only\n");
    return false;
  } else if (cMode == WIFI_MODE_STA) {
    Log(logger::S_TIME, "adopt(): Will adopt i am STA only\n");
    return true;
  }
